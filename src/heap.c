...
unsigned long long int stat_percolations = 0;
...
short less_than(snode* tmp1, snode* tmp2){
	int i;
	for (i = 0; i < nobjs; i++){
		if (tmp1->g[i] + graph_node[tmp1->state].h[i] <= tmp2->g[i] + graph_node[tmp2->state].h[i]){ 
			if (tmp1->g[i] + graph_node[tmp1->state].h[i] < tmp2->g[i] + graph_node[tmp2->state].h[i])
				return 1;
		}else
			return 0;
...
//      if (child != heapsize && heap[child + 1]->key < heap[child]->key)
      if (child != heapsize && less_than(heap[child + 1],heap[child]))
        ++child;
//      if (heap[child]->key < tmp->key) {
      if (less_than(heap[child],tmp)) {
...
//    for (; hole > 1 && tmp->key < heap[hole / 2]->key; hole /= 2) {
    for (; hole > 1 && less_than(tmp,heap[hole / 2]); hole /= 2) {
...
//    if (hole > 1 && heap[hole / 2]->key > tmp->key)
    if (hole > 1 && less_than(tmp, heap[hole / 2]))
